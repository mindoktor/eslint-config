...
// test/fixtures/fail/. The root eslint.config.ts globally ignores these
// fixtures (they are deliberately broken), so the drift harness runs ESLint
// with THIS config instead, pointed at the fixtures' own tsconfig so the
// typed rules resolve types.
//
